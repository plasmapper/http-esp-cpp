...
public:
  /// @brief Get the transaction network stream
  /// @return network stream
  virtual std::shared_ptr<NetworkStream> GetNetworkStream() = 0;
  
  /// @brief Read the request body
  ...
  /// @brief Write the response with the status code 200
  /// @param body body
  /// @param bodySize body size
  /// @return error code
  ...
  /// @brief Write the response with no body
  /// @param statusCode status code
  /// @return error code
  esp_err_t WriteResponse (uint16_t statusCode);

  /// @brief Get the request HTTP method
  /// @return HTTP method
